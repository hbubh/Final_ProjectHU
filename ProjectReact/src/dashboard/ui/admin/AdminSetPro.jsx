...
const AdminSetPro = ({ setDis, thisDis }) => {
  const [thisValue, setVlue] = React.useState("");
  const [thisAble, setAble] = React.useState(true);
  const [thisCircle, setCircle] = React.useState(" Upgrade User To Pro Plan");
  ...
  const handleSetBuzClick = async () => {
    try {
      console.log("hi");
      const dataFromServer = await axios.patch(`/users/plan/pro`, {
        email: thisValue,
      });
      setCircle(circle);
      toast(`You Upgrade User: ${thisValue} to Pro successfully . `, {
        position: "top-center",
        autoClose: 3500,
      ...
      setDis(!thisDis);
      setTimeout(() => {
        setCircle(" Upgrade User To Pro Plan");
      }, 3000);
    } catch (err) {
